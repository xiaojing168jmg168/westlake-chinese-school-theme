...
class Like {
  constructor() {
    if (document.querySelector('.like-box')) {
      (axios__WEBPACK_IMPORTED_MODULE_0___default().defaults.headers.common["X-WP-Nonce"]) = wcsData.nonce;
      this.events();
    }
  }
  events() {
  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(wcsData.root_url + '/wp-json/university/v1/manageLike', {
  ...
  async deleteLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()({
        url: wcsData.root_url + '/wp-json/university/v1/manageLike',
        method: 'delete',
        data: {
...
class MyNotes {
  ...
  async deleteNote(e) {
    const thisNote = this.findNearestParentLi(e.target);
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](wcsData.root_url + '/wp-json/wp/v2/note/' + thisNote.getAttribute('data-id'));
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(wcsData.root_url + '/wp-json/wp/v2/note/' + thisNote.getAttribute('data-id'), ourUpdatedPost);
      this.makeNoteReadOnly(thisNote);
    } catch (e) {
      console.log('Sorry');
    }
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(wcsData.root_url + '/wp-json/wp/v2/note/', ourNewPost);
...
class Search {
  ...
  getResults() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(wcsData.root_url + '/wp-json/wcs/v1/search?term=' + this.searchField.val(), results => {
      this.resultsDiv.html(`
     <div class="row">
        <div class="one-third">
        ...
        ${results.instructors.length ? '<ul class="professor-cards">' : `<p>No instructors matches that search.</p>`}
    ...
    <span class="professor-card__name">${item.title}</span>
        ...
        ${results.events.length ? '' : `<p>No events matches that search. <a href="${wcsData.root_url}/events">View all events.</a></p>`}
